                    ...
                    <div class="rt-post">
                        <div class="rt-heading">
                            ...
                            <div style="padding-left: 450px">
                                <a href="{{route('user.hire.new.tool',$tool->id)}}"
                                    style=" background-color: #04AA6D;border: none;color: white;padding: 15px 32px;
                                    text-align: center;text-decoration: none;display: inline-block;font-size: 16px;
                                    border-radius: 10px" type="button" >Hire A Tool
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="rt-postcontent">
                            ...
                            <div class="rt-actionbtns">
                                <form action="{{route('user.like.tool',$tool->id)}}" method="post">
                                    @csrf
                ...
                <div class="modal-body">
                    <div class="rt-contentboxmodal">
                        ...
                        <form class="rt-themefrom at-formapplyovertime at-formapplyrating"
                              action="{{route('user.review.store')}}" method="post"
                              enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="tool_id" value="{{$tool->id}}">
                            <div class="form-body">
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            $('#toolPricingButton').on('click', function (e) {
                e.preventDefault();
            ...
            $('.submitReplyBtn').on('click', function (e) {
                ...
                let url = '{{route('user.submit.reply')}}';
            ...
            $('.toolHiringBtn').on('click', function (e) {
                e.preventDefault();
                let price_id = $(this).attr('data-price-id');
                let tool_id = $(this).attr('data-tool-id');
                let data = {
                    '_token': "{{csrf_token()}}",
                    'tool_id': tool_id,
                    'price_id': price_id,
                }
                let url = '{{route('user.hire.tool')}}';
                $.post(url, data, function (res) {
                    if (res.status === 'success') {
